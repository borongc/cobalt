...
bool MojoRendererBypassBridge::IsActive() {
  base::AutoLock auto_lock(lock_);
  return active_;
}

AudioDecoderConfig MojoRendererBypassBridge::GetAudioConfig() {
  base::AutoLock auto_lock(lock_);
...
VideoDecoderConfig MojoRendererBypassBridge::GetVideoConfig() {
  base::AutoLock auto_lock(lock_);
...
bool MojoRendererBypassBridge::SupportsConfigChanges(DemuxerStream::Type type) {
  base::AutoLock auto_lock(lock_);
  if (!active_) {
    return false;
  }
...
StreamLiveness MojoRendererBypassBridge::GetLiveness(DemuxerStream::Type type) {
  base::AutoLock auto_lock(lock_);
  if (!active_) {
