...
#if BUILDFLAG(USE_STARBOARD_MEDIA)
// A proxy DemuxerStream that forwards Read calls to the MojoRendererBypassBridge.
...
class ProxyDemuxerStream : public DemuxerStream {
 public:
  ProxyDemuxerStream(scoped_refptr<MojoRendererBypassBridge> bridge, DemuxerStream::Type type)
...
void MojoRendererService::InitializeWithStreamPointers(
  ...
  if (!bypass_bridge_) {
    LOG(WARNING) << __func__ << ": Bypass bridge not found for ID " << bypass_bridge_id;
  ...
  if (audio_config.IsValidConfig()) {
    audio_proxy = std::make_unique<ProxyDemuxerStream>(bypass_bridge_, DemuxerStream::AUDIO);
  }
  VideoDecoderConfig video_config = bypass_bridge_->GetVideoConfig();
  if (video_config.IsValidConfig()) {
    video_proxy = std::make_unique<ProxyDemuxerStream>(bypass_bridge_, DemuxerStream::VIDEO);
  }

  media_resource_ = std::make_unique<ProxyMediaResource>(std::move(audio_proxy), std::move(video_proxy));
